...
$arrTop = $sAPI->getTopSuggestions();
for ($i = 0; $i < count($arrTop); $i++) {
  if ($arrTop[$i] !== "") {
    $detail = $sAPI->getSuggestionsDetails($arrTop[$i]);
    foreach ($detail as $details) {
      ...
      $top3 = $top3 . "<a href='#' class='text-muted' style='text-decoration: none;'><div class='d-flex text-muted pt-3'><svg class='bd-placeholder-img flex-shrink-0 me-2 rounded' width='32' height='32' xmlns='http://www.w3.org/2000/svg' role='img' aria-label='Placeholder: 32x32' preserveAspectRatio='xMidYMid slice' focusable='false'><title>Placeholder</title><rect width='100%' height='100%' fill='#007bff' /><text x='50%' y='50%' fill='#007bff' dy='.3em'>32x32</text></svg><div class='pb-3 mb-0 small lh-sm border-bottom w-100'><div class='d-flex justify-content-between'><strong class='text-primary'>".$sTitle."</strong><span>Vote: ". $voteCount ."</span></div><span class='d-block text-muted'>@".$userUsername."</span><span>". $sCreatedDate ."</span></div></div></a>";
    }
  }
}

// show 5 recent suggestions
$arrRecent = $sAPI->getFiveRecentSuggestions();
foreach ($arrRecent as $arrRecents) {
  $sId = $arrRecents['SUGGESTIONS_ID'];
  $sTitle = $arrRecents['SUGGESTIONS_TITLE'];
  $sDetails = $arrRecents['SUGGESTIONS_DETAILS'];
  $sCreatedDate = $arrRecents['SUGGESTIONS_CREATED_DATE'];
  $cCreatedBy = $arrRecents['USER_NRIC'];
  $userUsername = $uAPI->getUserUsername($cCreatedBy);
  $voteCount = $sAPI->getVote($sId);

  $recent3 = $recent3."<a href='#' class='text-muted' style='text-decoration: none;'><div class='d-flex text-muted pt-3'><svg class='bd-placeholder-img flex-shrink-0 me-2 rounded' width='32' height='32' xmlns='http://www.w3.org/2000/svg' role='img' aria-label='Placeholder: 32x32' preserveAspectRatio='xMidYMid slice' focusable='false'><title>Placeholder</title><rect width='100%' height='100%' fill='#007bff' /><text x='50%' y='50%' fill='#007bff' dy='.3em'>32x32</text></svg><div class='pb-3 mb-0 small lh-sm border-bottom w-100'><div class='d-flex justify-content-between'><strong class='text-primary'>".$sTitle."</strong><span>Vote: ". $voteCount ."</span></div><span class='d-block text-muted'>@".$userUsername."</span><span>". $sCreatedDate ."</span></div></div></a>";
}
    ...
    <div class="container-fluid">
      ...
      <div class="navbar-collapse offcanvas-collapse" id="navbarsExampleDefault">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          ...
          <li class="nav-item">
            <a class="nav-link active" href="#">Suggestions</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="#">Volunteer Program</a>
          </li>
          <li class="nav-item">
  ...
  <main class="container">
    ...
    <div class="my-3 p-3 bg-body rounded shadow-sm">
      ...
      <small class="d-block text-end mt-3">
        <a href="#">See more top suggestions</a>
      </small>
    </div>
