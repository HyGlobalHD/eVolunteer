...
$sessiongroupcode = $_SESSION["groupcode"];
...
if(isset($_GET['username'])) {
    $searchnric = $uAPI->getUserNRIC($_GET['username']);
    if(!(is_null($searchnric))) {
        foreach($searchnric as $s) {
            $a = $s['USER_NRIC'];
        }
    }
}

if($_SERVER["REQUEST_METHOD"] == "POST" &&  isset($_POST['banBtn'])) {
    if($uAPI->updateUserStatus($a, "B")) {
        $msgt = $msgt . $sAPI->msgbox(1, "User banned successfully.");
    } else {
        $msgt = $msgt . $sAPI->msgbox(0, "Failed to ban user.");
    }
}
if($_SERVER["REQUEST_METHOD"] == "POST" &&  isset($_POST['unbanBtn'])) {
    if($uAPI->updateUserStatus($a, "A")) {
        $msgt = $msgt . $sAPI->msgbox(1, "User unbanned successfully.");
    } else {
        $msgt = $msgt . $sAPI->msgbox(0, "Failed to unban user.");
    }
}
...
<head>
    ...
    <title>eVolunteer - User Profile</title>
    ...
    <main class="container">
                ...
                <div class="pb-3 mb-0 lh-sm border-bottom w-100">
                    <div class="">
                        ...
                        if(isset($fullname) && $fullname != ""){
                            $formatName = "<strong class='text-dark'>Full Name: </strong><span class='text-dark'>$fullname</span>";
                            echo $formatName;
                        }
                        ?>
                    </div>
                    ...
                    ?>
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"])."?username=". $_GET['username']; ?>"
                        method="POST" onsubmit="return confirm('Are you sure you want to ban/unban the user?');">
                        <?php
                    $i = "";
                    if($sessiongroupcode == "ADM" && strlen($a) > 0) {
                        if($a !== $currUser) {
                            if($uAPI->getUserStatus($a) == "A") {
                                $i = "<input type='submit' name='banBtn' value='Ban User' class='btn btn-danger'>";
                            } else {
                                $i = "<input type='submit' name='unbanBtn' value='unBan User' class='btn btn-warning'>";
                            }
                        } 
                    }
                    echo $i;
        ...
        <div class="my-3 p-3 bg-body rounded shadow-sm">
            ...
            <small class="d-block mt-3">
                <strong>Top Voted Suggestions:</strong>
            </small>
            ...
            <small class="border-top d-block text-end mt-3">
                <a class="text-primary" style="text-decoration: none;"
                    href="search_suggestions.php?username=<?php echo $username; ?>">Search more</a>
            </small>
        </div>
        </div>
        <div class="my-3 p-3 bg-body rounded shadow-sm">
