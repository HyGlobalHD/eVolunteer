...
// note: to check groupcode is equal to ORG or ADM
if ($_SESSION['groupcode'] == "USER") {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" &&  isset($_POST['createVolunteerProgram'])) {
  ...
  if (!(is_null($vp_details)) && strlen(trim($vp_details)) > 0 && !(is_null($vp_title)) && strlen(trim($vp_title)) > 0) {
    if ($vp_start_date > $vp_end_date) {
      $volunteerssectionMSG = $sAPI->msgbox(3, "Opsie! Something wrong happen! Try again. Please make sure the start date is before the end date.");
    } else {
...
<head>
  ...
  <title>eVolunteer - New Volunteer Program</title>
  ...
  <main class="container">
    ...
    <div class="my-3 p-3 bg-body rounded shadow-sm">
      ...
      <form class="border-bottom my-3" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . "?sid=" . $sid; ?>" method="POST">
        ...
        <div class="form-floating">
          <input type="date" class="form-control" id="vp_start_date" name="vp_start_date" placeholder="volunteers start date" autocomplete="off" min="<?php $date = strtotime("+7 day", strtotime(date("Y-m-d")));
                                                                                                                                                      echo date("Y-m-d", $date); ?>" required>
          <label for="vp_start_date">volunteers start date</label>
        </div>
        <div class="form-floating">
          <input type="date" class="form-control" id="vp_end_date" name="vp_end_date" placeholder="volunteers end date" autocomplete="off" min="<?php $date = strtotime("+7 day", strtotime(date("Y-m-d")));
                                                                                                                                                echo date("Y-m-d", $date); ?>" required>
